...
public class TransactionService {
	...
	public String showShoppingCart() {
		...
		if (transactionD.shoppingCartList().isEmpty()) {
			...
			for (Product prod : transactionD.shoppingCartList()) {
				response += "Item:" + (transactionD.shoppingCartList().indexOf(prod) + 1) + ", SKU: " + prod.getSku()
						+ ", Quantidade: " + prod.getQtt() + ", Preço total: " + prod.getSalePrice() + "\n";
			}
		}
		...
		if (transactionD.registerTransactionHistory(transaction)) {
			...
			if (transaction.getPaymentMethod() == PaymentMethod.DEBIT) {
				response += "\nPreço final: " + transaction.getFinalPrice() + "\nMetôdo de pagamento: "
						+ transaction.getPaymentMethod().getDescripiton() + "\nNúmero do cartão: "
	...
	public String showSaleHistory() {
		...
		if (transactionD.showSaleHistory().isEmpty()) {
			response = "Ainda não possui historico de vendas";
		} else {
			for (Transaction transaction : transactionD.showSaleHistory()) {
				response += "Venda: " + (transactionD.showSaleHistory().indexOf(transaction)+1);
				if (transaction.getPaymentMethod() == PaymentMethod.DEBIT) {
					response += "\nPreço final: " + transaction.getFinalPrice() + "\nMetôdo de pagamento: "
							+ transaction.getPaymentMethod().getDescripiton() + "\nNúmero do cartão: "
							+ transaction.getCardData();
